...
class TransD(Model):
    def __init__(
        ...
        self.ent_embeddings = nn.Embedding(self.ent_tot, self.dim_e, device=devices)
        self.rel_embeddings = nn.Embedding(self.rel_tot, self.dim_r, device=devices)
        self.ent_transfer = nn.Embedding(self.ent_tot, self.dim_e, device=devices)
        self.rel_transfer = nn.Embedding(self.rel_tot, self.dim_r, device=devices)
    ...
    def _transfer(self, e, e_transfer, r_transfer):
        if e.shape[0] != r_transfer.shape[0]:
            ...
            e = F.normalize(
                self._resize(e, -1, r_transfer.size()[-1])
                + torch.sum(e * e_transfer, -1, True) * r_transfer,
                p=2,
                dim=-1,
            )
    ...
    def predict_entropy(self, data):
        score = self.forward_entropy(data)
        if self.margin_flag:
            score = self.margin - score
            return score
        else:
    ...
    def forward_entropy(self, data):
        ...
        h = self._transfer(h, h_transfer, r_transfer)
        t = self._transfer(t, t_transfer, r_transfer)
        score = self._calc_entropy(h, t, r, mode)
        if self.margin_flag:
            return self.margin - score
        else:
            return score

    def _calc_entropy(self, h, t, r, mode):
        if self.norm_flag:
            h = F.normalize(h, 2, -1)
            r = F.normalize(r, 2, -1)
            t = F.normalize(t, 2, -1)
